...
setInterval(function(){$('#matchCount').html($('#content .cheat_sheet_row:visible').length);},1000);if($.cookie('selectedLanguages')){selectedLanguages=$.cookie('selectedLanguages').split('-');for(var i=0;i<selectedLanguages.length;i++){$('#langselect-'+selectedLanguages[i]).prop('checked',true);}}else{if(defaultLanguages){for(var i=0;i<defaultLanguages.length;i++){$('#langselect-'+defaultLanguages[i]).prop('checked',true);}}}
if(rememberTagCookieName!=''){if($.cookie(rememberTagCookieName)){selectedFavouriteTags=$.cookie(rememberTagCookieName).split('-');for(var i=0;i<selectedFavouriteTags.length;i++){$('#tagselect-'+selectedFavouriteTags[i]).prop('checked',true);}}}
if((rememberTagCookieName+'Rating')!=''){if($.cookie(rememberTagCookieName+'Rating')){selectedRatings=$.cookie(rememberTagCookieName+'Rating').split('-');for(var i=0;i<selectedRatings.length;i++){$('#ratingselect-'+selectedRatings[i]).prop('checked',true);}}}
...
$('#ratingStarsCount').show();$('#ratingStarsCount a').html(ratingCount);});});if(deviceBracket=='desktop'){$('.footer_popup_inner').append('<a href="#" class="imagelink footer_popup_close"><i class="fa fa-close"></i></a>');$('.footer_popup_close').click(function(){$('.footer_popup').slideUp();return false;});setTimeout(function(){$('.footer_popup').slideDown();},5000);var menuPadding=$('#cs_menu .download_menu').width();$('#pagetitle h1').css('paddingRight',menuPadding);}});function processTags(){var to_show='';var to_show_lang='';var to_show_rating='';var taglist='';var langlist='';var ratinglist='';var tagFilterFound=false;var langFilterFound=false;var ratingFilterFound=false;var filterFound=false;var selectedFavouriteTags=[];var selectedLanguages=[];var selectedRatings=[];var activeTagCount=0;$('input.tagoptions:checked').each(function(){activeTagCount=activeTagCount+1;filterFound=true;tagFilterFound=true;if(to_show.length>0){to_show=to_show+',';taglist=taglist+', ';}
...
function checkContrast(value,element){var colour=value.replace('#','');var rdec=parseInt(colour.substring(0,2),16);var gdec=parseInt(colour.substring(2,4),16);var bdec=parseInt(colour.substring(4,6),16);var contrast=(1.05 /(0.05+(0.2126*Math.pow(rdec /255,2.2))+(0.7152*Math.pow(gdec /255,2.2))+(0.0722*Math.pow(bdec /255,2.2))));if(contrast<2){element.css('color','#000000');}else{element.css('color','#FFFFFF');}
if(contrast<1.5){return false;}else{return true;}}